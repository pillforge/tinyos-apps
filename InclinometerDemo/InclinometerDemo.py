#
# This class is automatically generated by mig. DO NOT EDIT THIS FILE.
# This class implements a Python interface to the 'InclinometerDemo'
# message type.
#

import tinyos.message.Message

# The default size of this message type in bytes.
DEFAULT_MESSAGE_SIZE = 2

# The Active Message type associated with this message.
AM_TYPE = 6

class InclinometerDemo(tinyos.message.Message.Message):
    # Create a new InclinometerDemo of size 2.
    def __init__(self, data="", addr=None, gid=None, base_offset=0, data_length=2):
        tinyos.message.Message.Message.__init__(self, data, addr, gid, base_offset, data_length)
        self.amTypeSet(AM_TYPE)
    
    # Get AM_TYPE
    def get_amType(cls):
        return AM_TYPE
    
    get_amType = classmethod(get_amType)
    
    #
    # Return a String representation of this message. Includes the
    # message type name and the non-indexed field values.
    #
    def __str__(self):
        s = "Message <InclinometerDemo> \n"
        try:
            s += "  [x=0x%x]\n" % (self.get_x())
        except:
            pass
        return s

    # Message-type-specific access methods appear below.

    #
    # Accessor methods for field: x
    #   Field type: short
    #   Offset (bits): 0
    #   Size (bits): 16
    #

    #
    # Return whether the field 'x' is signed (False).
    #
    def isSigned_x(self):
        return False
    
    #
    # Return whether the field 'x' is an array (False).
    #
    def isArray_x(self):
        return False
    
    #
    # Return the offset (in bytes) of the field 'x'
    #
    def offset_x(self):
        return (0 / 8)
    
    #
    # Return the offset (in bits) of the field 'x'
    #
    def offsetBits_x(self):
        return 0
    
    #
    # Return the value (as a short) of the field 'x'
    #
    def get_x(self):
        return self.getSIntElement(self.offsetBits_x(), 16, 1)
    
    #
    # Set the value of the field 'x'
    #
    def set_x(self, value):
        self.setSIntElement(self.offsetBits_x(), 16, value, 1)
    
    #
    # Return the size, in bytes, of the field 'x'
    #
    def size_x(self):
        return (16 / 8)
    
    #
    # Return the size, in bits, of the field 'x'
    #
    def sizeBits_x(self):
        return 16
    
